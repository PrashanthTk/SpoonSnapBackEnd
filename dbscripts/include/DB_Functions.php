...
class DB_Functions {
    ...
    public function store_report($messageid, $dish_id, $report_time, $latitude, $longitude, $restaurant_name, $revgeo_full, $food_timing, $tastiness, $rating, $username, $first_name, $imgurl, $weather, $North_Indian, $South_Indian, $Chinese, $Mughlai, $Fast_Food, $Desserts, $Continental, $East_Indian, $West_Indian, $serving) {
        /*$uuid = uniqid('', true);
        $hash = $this->hashSSHA($password);
        $encrypted_password = $hash["encrypted"]; // encrypted password
        $salt = $hash["salt"]; // salt*/
 
        $stmt = $this->conn->prepare("INSERT INTO microreports (messageid, report_time, latitude, longitude, restaurant_name, revgeo_full, food_timing, tastiness, rating, username, first_name, imgurl, weather, North_Indian, South_Indian, Chinese, Mughlai, Fast_Food, Desserts, Continental, East_Indian, West_Indian, serving) VALUES(?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?,?, ?, ? )");
        $stmt->bind_param("sssssssssssssssssssssss",$messageid, $report_time, $latitude, $longitude, $restaurant_name, $revgeo_full, $food_timing, $tastiness, $rating, $username, $first_name, $imgurl, $weather, $North_Indian, $South_Indian, $Chinese, $Mughlai, $Fast_Food, $Desserts, $Continental, $East_Indian, $West_Indian, $serving);
        $result = $stmt->execute();
        $stmt->close();
        $store_report_return_value=array('$report_time','$restaurant_name','$latitude','$longitude','$rating');
        return $store_report_return_value;
        // check for successful store
        /*if ($result) {
            $stmt = $this->conn->prepare("SELECT * FROM users WHERE email = ?");
            $stmt->bind_param("s", $email);
